...
## 插件
Mybatis插件实现主要利用了JDK动态代理和责任链模式,并且只支持对Executor、ParameterHandler、ResultSetHandler、StatementHandler四种接口.实现一个插件，需要实现interceptor接口，并且在mybatis配置文件中通过<plugin>标签配置自定义的插件。
